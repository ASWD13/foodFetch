      ...
      <Dialog>
        ...
        <div className="bg-slate-900 md:hidden">
          <div className="flex justify-center p-4">
            {!menuOpen ? <BiMenu className="text-white absolute top-4 right-4" size="30" onClick={toggleMenu} /> :
              <CgClose className="text-white absolute top-4 right-4" size="30" onClick={toggleMenu} />}
            <span className="ml-3 text-xl xl:block text-white">
              {"</>"}Food_Fetch
            </span>
          </div>
        ...
        <DialogContent className="h-full" >
          <h2 className="text-gray-900 text-3xl font-medium title-font mb-5 text-center">
            {authScreen == "login" ? "Welcome Back Foodiee" : "Lets Register"}
          </h2>
          {authScreen == "login" ? <LoginComponent /> : <RegisterComponent />}
          <p className="text-xs text-gray-500 ">

            {authScreen == "login" ? <span onClick={() => setAuthScreen("register")}>click here to register</span> : <span onClick={() => setAuthScreen("login")}>back to login</span>}
